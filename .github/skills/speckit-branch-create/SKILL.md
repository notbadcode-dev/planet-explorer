...
- **Riesgo de pérdida silenciosa si `/speckit-specify` se reinvoca sobre la misma feature**: este hook puede recrear la rama de feature desde `develop` si vuelve a ejecutarse (p. ej. el usuario repite `/speckit-specify`, o algo relanza `create-feature-branch.sh`). Si `spec.md`/`checklists/requirements.md` de un turno anterior no se habían comprometido con `git commit`, se pierden sin ningún aviso (`git status` queda "clean" porque nunca hubo commit). Por eso, tras crear `spec.md` + checklists y actualizar `.specify/feature.json`, el flujo de `/speckit-specify` MUST hacer `git commit` de inmediato, antes de reportar la finalización al usuario — no dejarlo para un futuro `/speckit-plan` o cierre de spec.
