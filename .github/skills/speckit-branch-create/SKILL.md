...
> Las ramas de feature nuevas SIEMPRE deben crearse desde la única rama principal del
> repositorio (`main` o `master`, autodetectada; nunca ambas a la vez).
...
3. Clasifica `CURRENT_BRANCH` (la rama principal se autodetecta: `origin/HEAD` → `main` local → `master` local; ver `create-feature-branch.sh`):
   - **Es la rama principal**: ve directamente al paso 5, sin `--related-feature`.
   - **Rama de feature** (coincide con `^[0-9]{3,}-`): ve al paso 4.
   - **Cualquier otro caso** (HEAD desacoplado, rama sin ese patrón): detente y pide al usuario que vuelva a la rama principal (`git checkout main` o `master`) antes de volver a ejecutar `/speckit-specify`. No crees la spec.
4. Cuando `CURRENT_BRANCH` es una rama de feature, decide si la nueva feature está relacionada:
   ...
   - Si NO está relacionada → dile al usuario que vuelva a la rama principal (`git checkout main` o `master`) y vuelva a lanzar `/speckit-specify`, y detente sin crear la spec.
...
- Este hook nunca decide él mismo la relación entre specs sin evidencia: ante la duda, siempre pregunta o exige `master`/`develop` como base. Esto evita crear árboles de ramas encadenadas por error.
