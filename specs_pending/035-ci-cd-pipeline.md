---
id: "035-ci-cd-pipeline"
name: "Pipeline de CI/CD"
phase: "Fase 4 — Gate de publicación estable (MVP)"
depends_on: ["034-automated-e2e-testing"]
---

# 035 — Pipeline de CI/CD (ci-cd-pipeline)

## Objetivo
Consolidar, como parte del Gate de publicación estable (Fase 4), un pipeline de integración y despliegue continuo que ejecute lint, tests unitarios y el smoke E2E (034) en cada cambio, con despliegue automatizado a producción (GitHub Pages) tras cada release. Esta es la versión MÍNIMA necesaria para publicar; la Fase 7 (ver 050-visual-regression-testing) la EXTENDERÁ más adelante añadiendo regresión visual, sin bloquear la primera publicación por su ausencia.

## Contexto / motivación
Con lint/test/build ya funcionando manualmente (`.github/workflows/ci.yml`) y el smoke E2E de 034 recién definido, este slice los conecta en un pipeline reproducible que bloquea el merge/despliegue ante regresiones, evitando que la validación dependa de ejecutarla manualmente antes de cada release. La regresión visual (050) es deliberadamente NO bloqueante para el primer release público: añadirla aquí introduciría una dependencia hacia una fase muy posterior (Fase 7) sin necesidad real todavía (principio VI).
...
- Punto de extensión documentado para que 050 (Fase 7) añada el job de regresión visual sin rediseñar el pipeline.
...
- Regresión visual automatizada (ver 050, Fase 7 — se añade a este mismo pipeline más adelante, no lo bloquea).
...
- Añadir el job de regresión visual (050) en el futuro no requiere rediseñar el pipeline.
